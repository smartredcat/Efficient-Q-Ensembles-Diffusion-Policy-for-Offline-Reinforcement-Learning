...
hyperparameters = {
    ...
    'hopper-medium-v2':              {'lr': 3e-4, 'eta': 1.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 9.0,  'top_k': 2},
    'walker2d-medium-v2':            {'lr': 3e-4, 'eta': 1.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 1.0,  'top_k': 1},
    'halfcheetah-medium-replay-v2':  {'lr': 3e-4, 'eta': 2.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 2.0,  'top_k': 0},
    'hopper-medium-replay-v2':       {'lr': 3e-4, 'eta': 1.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 4.0,  'top_k': 2},
    'walker2d-medium-replay-v2':     {'lr': 3e-4, 'eta': 1.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 4.0,  'top_k': 1},
    'halfcheetah-medium-expert-v2':  {'lr': 3e-4, 'eta': 2.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 50, 'num_epochs': 1000, 'gn': 7.0,  'top_k': 0},
    ...
    'antmaze-umaze-v0':              {'lr': 3e-4, 'eta': 2.0,   'max_q_backup': False,  'reward_tune': 'cql_antmaze', 'eval_freq': 50, 'num_epochs': 1000, 'gn': 2.0,  'top_k': 2},
    ...
    'antmaze-medium-diverse-v0':     {'lr': 3e-4, 'eta': 2.0,   'max_q_backup': True,   'reward_tune': 'cql_antmaze', 'eval_freq': 50, 'num_epochs': 1000, 'gn': 1.0,  'top_k': 1},
...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    while (training_iters < max_timesteps) and (not early_stop):
        ...
        train_log["train/epoch"] = curr_epoch
        train_log["train/BC Loss"] = np.mean(loss_metric['bc_loss'])
        train_log["train/QL Loss"] = np.mean(loss_metric['ql_loss'])
        train_log["train/Actor Loss"] = np.mean(loss_metric['actor_loss'])
        train_log["train/Critic Loss"] = np.mean(loss_metric['critic_loss'])
#         wandb.log(train_log)
        ...
        logger.dump_tabular()
        r_score = eval_norm_res*100
        temp.append(r_score)
        show_image(temp,'normalize_score',tt,args.env_name,curr_epoch,args)
        bc_loss = np.mean(loss_metric['bc_loss'])
        if args.early_stop:
            early_stop = stop_check(metric, bc_loss)

        metric = bc_loss
        val_log["test/score"]=0
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    utils.print_banner(f"Evaluation over {eval_episodes} episodes: {avg_reward:.4f} {avg_norm_score:.4f}")
    return avg_reward, std_reward, avg_norm_score, std_norm_score

def show_image(x, s,t,name,episod,args):
    plt.figure(figsize=(14, 8), dpi=100)
    episod = str(episod)
    if not os.path.exists("pic"):
        os.mkdir("pic")
        
    parent_folder = os.path.join("pic",f"{args.env_name} T:{args.T} LCBcoef:{args.lcb_coef} n:{args.num_critics} rand:{args.rand} time:{t}")
    if not os.path.exists(parent_folder):
            os.mkdir(parent_folder)
    ex = os.path.join(parent_folder,f"{name}_{episod}")
    if not os.path.exists(ex):
            os.mkdir(ex)
    new_x = [i * 50 for i in range(1, len(x) + 1)]
    plt.plot(new_x, x)
    plt.xlabel('epochs')
    plt.ylabel(s)
    timestamp =str(int(time.time()))
    
    f_n = os.path.join(ex,f"plt_{name}_{timestamp}.png")
    plt.savefig(f_n)
    plt.close()
...
#     n=args.env_name+"T:"+arg.T+"::LCBcoef:"+arg.lcb_coef+"n:"+args.num_critics
    tt = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    n=f"{args.env_name} T:{args.T} LCBcoef:{args.lcb_coef} n:{args.num_critics} rand:{args.rand} time:{tt}"
    #wandb.init(project="E2DP",name=n)
    train_log={}#训练日志
    val_log={}#评估日值
    ...
    # Setup Logging
    file_name = f"{args.env_name}|{args.exp}|E-E-diffusion-{args.algo}|T-{args.T}"
    ...
    variant = vars(args)
    variant.update(version=f"E-E-Diffusion-Policies-RL")
